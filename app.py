...
def parse_cards(cards_text):
    ...
    for line in cards_text.strip().split("\n"):
        line = line.strip()
        if line and re.match(r"^\d{13,19}\|\d{1,2}\|\d{2,4}\|\d{3,4}$", line):
            cards.append(line)
    return cards
...
def categorize_error(error_msg, status_code=None):
    ...
    elif "dns" in error_msg or "name" in error_msg:
    ...
    elif "stripe key" in error_msg:
        return "Stripe Key Not Found"
    elif "invalid card" in error_msg:
        return "Invalid Card Format"
    elif "3d" in error_msg or "3ds" in error_msg:
        return "3D Secure Required"
    elif "insufficient" in error_msg:
        return "Insufficient Funds"
    elif "expired" in error_msg:
        return "Card Expired"
    elif "incorrect" in error_msg and "cvc" in error_msg:
...
def get_stripe_key(domain):
    ...
    for attempt in range(MAX_RETRIES):
        for url in urls_to_try:
            try:
                logger.debug(f"Trying URL (attempt {attempt+1}): {url}")
                ...
                logger.warning(f"Timeout getting Stripe key from {url}")
                continue
            except requests.exceptions.ConnectionError as e:
                logger.warning(f"Connection error getting Stripe key from {url}: {e}")
                continue
...
def process_card_enhanced(domain, ccx, use_registration=True):
    ...
    if not stripe_key:
        return {
            "Response": "Failed to obtain Stripe key",
            "Status": "Declined",
            "ErrorType": "Stripe Key Missing",
        }
    if use_registration:
    ...
    for url in payment_urls:
        try:
            ...
            logger.warning(f"Timeout getting nonce from {url}")
            continue
        except requests.exceptions.ConnectionError:
            logger.warning(f"Connection error getting nonce from {url}")
            continue
        except Exception as e:
    ...
    try:
        ...
        return {
            "Response": "Payment method creation timeout",
            "Status": "Declined",
            "ErrorType": "Timeout",
        }
    except requests.exceptions.ConnectionError:
        return {
            "Response": "Payment method creation connection error",
            "Status": "Declined",
        ...
        return {
            "Response": f"Payment Method Creation Failed: {str(e)[:100]}",
            "Status": "Declined",
    ...
    for endpoint in endpoints:
        for data_payload in data_payloads:
            try:
                logger.debug(f"Trying endpoint: {endpoint['url']} with payload keys: {list(data_payload.keys())}")
                ...
                try:
                    ...
                    logger.debug(f"Setup raw response: {setup_response.text[:500]}")
                if setup_data.get("success", False):
                    ...
                    if data_status == "requires_action":
                        ...
                        return {
                            "Response": "3D Secure Authentication Required",
                            "Status": "Declined",
                            "ErrorType": "3D Secure",
                        }
                ...
                last_error = {
                    "Response": "Setup intent connection error",
                    "Status": "Declined",
                    "ErrorType": "Connection Error",
                }
                continue
                ...
                last_error = {
                    "Response": f"Setup error: {str(e)[:100]}",
                    "Status": "Declined",
                    "ErrorType": categorize_error(e),
                }
                continue
...
def process_single_check(job_id, domain, card, card_index, total_cards):
    try:
        ...
        logger.error(f"Error in process_single_check: {e}")
        result_entry = {
            "Card": card,
            "Domain": domain,
            "Response": str(e)[:200],
            ...
            for domain in domains:
                for card in cards:
                    future = executor.submit(
                        ...
                        job_id, domain, card, task_index, total_tasks,
                    )
...
<title>AutoStripe API Pro - DEVELOPER: @Awmtee</title>
...
.result-success{border-left:5px solid var(--success);}
.result-error{border-left:5px solid var(--error);}
...
.result-item.info{border-left:5px solid var(--info);}
...
<div class="tab" onclick="switchTab('mass')">Mass Checker Pro</div>
...
def mass_check():
    try:
        ...
        logger.debug(f"Mass check request: key={key}, sites={len(sites)}, cards={len(cards)}")
        if key != "inferno":
            return jsonify({"error": "Invalid API key", "status": "Unauthorized"}), 401
...
def mass_status():
    try:
        ...
        logger.error(f"Mass status error: {e}")
        return jsonify({"error": f"Internal server error: {str(e)}", "status": "Error"}), 500
...
def mass_results():
    try:
        ...
        logger.error(f"Mass results error: {e}")
        return jsonify({"error": f"Internal server error: {str(e)}", "status": "Error"}), 500
...
def export_csv():
    try:
        ...
        return Response(
            csv_data,
            mimetype="text/csv",
            headers={"Content-Disposition": f"attachment; filename=mass_check_{job_id}.csv"}
        )
    except Exception as e:
...
def export_json():
    try:
        ...
        return Response(
            ...
            headers={"Content-Disposition": f"attachment; filename=mass_check_{job_id}.json"}
        )
    except Exception as e:
