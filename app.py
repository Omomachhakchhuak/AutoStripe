...
.tab{padding:12px 25px;margin:0 10px 10px;background:var(--glass);backdrop-filter:blur(10px);border:1px solid var(--glass-border);border-radius:30px;cursor:pointer;transition:all 0.3s ease;font-weight:500;user-select:none;-webkit-tap-highlight-color:transparent;}
...
.form-control{width:100%;padding:15px;border-radius:10px;border:1px solid var(--glass-border);background:rgba(255,255,255,0.05);color:white;font-family:'Poppins',sans-serif;transition:all 0.3s ease;font-size:16px;}
.form-control:focus{outline:none;border-color:var(--primary);background:rgba(255,255,255,0.1);}
.form-control::placeholder{color:rgba(255,255,255,0.7);}
textarea.form-control{min-height:150px;resize:vertical;}
.btn{display:inline-block;padding:12px 25px;border-radius:10px;border:none;font-weight:600;cursor:pointer;transition:all 0.3s ease;text-align:center;font-family:'Poppins',sans-serif;margin-right:10px;margin-bottom:10px;font-size:16px;touch-action:manipulation;}
...
<div class="designer">DEVELOPER: @Awmtee</div>
<div><span class="status-indicator status-online"></span>API Status: Online | Max Cards: 50,000 | Max Sites: 100</div>
</header>
<div class="tabs">
<div class="tab active" data-tab="single">Single Checker</div>
<div class="tab" data-tab="mass">Mass Checker Pro</div>
...
<button class="btn btn-primary" id="btn-check-single">
<span id="single-loader" style="display:none;" class="loader"></span>Check Card
</button>
<button class="btn btn-secondary" id="btn-clear-single">Clear Results</button>
...
<button class="btn btn-primary" id="mass-check-btn">
<span id="mass-loader" style="display:none;" class="loader"></span>Start Mass Check
</button>
<button class="btn btn-secondary" id="btn-clear-mass">Clear Results</button>
<button class="btn btn-success" id="export-btn" style="display:none;">
<i class="fas fa-download"></i> Export CSV
</button>
<button class="btn btn-info" id="export-json-btn" style="display:none;">
...
<select id="filter-status"><option value="all">All Status</option><option value="Approved">Approved</option><option value="Declined">Declined</option><option value="Error">Error</option></select>
<select id="filter-domain"><option value="all">All Domains</option></select>
...
<button class="copy-btn" data-copy="/process?key=inferno&amp;site=example.com&amp;cc=card_number|mm|yy|cvv">Copy</button>
</div>
<div class="result-item">
...
<button class="copy-btn" data-copy="POST /mass_check with JSON body">Copy</button>
</div>
<div class="result-item">
...
<button class="copy-btn" data-copy="/mass_status?key=inferno&amp;job_id=">Copy</button>
</div>
<div class="result-item">
...
<button class="copy-btn" data-copy="/mass_results?key=inferno&amp;job_id=">Copy</button>
</div>
<div class="result-item">
...
<button class="copy-btn" data-copy="/export_csv?key=inferno&amp;job_id=">Copy</button>
</div>
<div class="result-item">
<h4>Health Check</h4>
...
<button class="copy-btn" data-copy="/health">Copy</button>
</div>
</div>
</div>
<footer>
<p>&copy; 2026 AutoStripe API Pro. All rights reserved. | DEVELOPER: @Awmtee</p>
</footer>
</div>
...
function copyToClipboard(text){
  navigator.clipboard.writeText(text).then(function(){showNotification('Copied to clipboard!','success');});
}
...
function checkSingleCard(){
  var site=document.getElementById('single-site').value.trim();
  var cc=document.getElementById('single-cc').value.trim();
  var resultContainer=document.getElementById('single-result');
  var resultContent=document.getElementById('single-result-content');
  var loader=document.getElementById('single-loader');
  ...
  .then(function(data){
    ...
    var cardParts=cc.split('|');
    var cardNumber=cardParts[0];
    var errorTypeHtml=data.ErrorType?'<div class="card-errortype">Error Type: '+data.ErrorType+'</div>':'';
    var cls=data.Status==='Approved'?'success':'error';
    resultContent.innerHTML='<div class="result-item '+cls+'"><div class="card-number">Card: '+formatCardNumber(cardNumber)+'</div><div class="card-domain">Domain: '+site+'</div><div class="card-response">Response: '+data.Response+'</div><div class="card-status">Status: '+data.Status+'</div>'+errorTypeHtml+'</div>';
...
function checkMassCards(){
  var sitesText=document.getElementById('mass-sites').value.trim();
  var ccText=document.getElementById('mass-cc').value.trim();
  var loader=document.getElementById('mass-loader');
  var btn=document.getElementById('mass-check-btn');
...
function startStatusPolling(){
  ...
  statusInterval=setInterval(function(){
      ...
      if(data.processed>0){
        ...
        .then(function(resultData){
          if(resultData.results){
            ...
            resultData.results.forEach(function(r){if(r.Domain)allDomains.add(r.Domain);});
            updateDomainFilter();
            filterResults();
          }
        });
    ...
    .catch(function(error){console.error('Status poll error:',error);});
  },2000);
}
...
function filterResults(){
  ...
  var domainFilter=document.getElementById('filter-domain').value;
  var searchFilter=document.getElementById('filter-search').value.toLowerCase();
  var tbody=document.getElementById('mass-result-content');
  tbody.innerHTML='';
  var filtered=currentResults.filter(function(r){
    if(statusFilter!=='all'&&r.Status!==statusFilter)return false;
    if(domainFilter!=='all'&&r.Domain!==domainFilter)return false;
    if(searchFilter){
      var cardNum=(r.Card||'').toLowerCase();
      var response=(r.Response||'').toLowerCase();
      var errType=(r.ErrorType||'').toLowerCase();
  ...
  filtered.forEach(function(result,index){
    ...
    var statusClass=result.Status==='Approved'?'badge-success':(result.Status==='Declined'?'badge-error':'badge-warning');
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  // Single checker
  document.getElementById('btn-check-single').addEventListener('click',checkSingleCard);
  document.getElementById('btn-clear-single').addEventListener('click',clearSingleResults);
  // Mass checker
  document.getElementById('mass-check-btn').addEventListener('click',checkMassCards);
  document.getElementById('btn-clear-mass').addEventListener('click',clearMassResults);
  document.getElementById('export-btn').addEventListener('click',exportResults);
  document.getElementById('export-json-btn').addEventListener('click',exportJSON);
  // Filters
  document.getElementById('filter-status').addEventListener('change',filterResults);
  document.getElementById('filter-domain').addEventListener('change',filterResults);
  document.getElementById('filter-search').addEventListener('input',filterResults);
  // Copy buttons
